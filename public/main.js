...
const projectVisual=v=>v.type==='bar'
  ?`<div class="readout-label">${v.label}</div><div class="readout-bar"><div class="readout-fill" style="width:${v.value}%"></div></div><div class="readout-value">${v.display}</div>`
  :`<div class="readout-label">${v.label}</div><div class="readout-chips">${v.items.map(x=>`<span class="readout-chip">${x}</span>`).join('')}</div>`;
document.getElementById('project-grid').innerHTML=projects.map((p,i)=>`<article class="project-card project-card--${i+1}"><div class="project-card-no">${p.id}</div><div class="project-visual">${projectVisual(p.visual)}</div><div class="project-content"><p class="project-eyebrow">${p.eyebrow}</p><h3>${p.name}</h3><p class="project-description">${p.description}</p><div class="project-proof">${p.outcome}</div><div class="tag-list project-tags">${tagList(p.stack,true)}</div><button type="button" class="project-more" data-project="${p.id}">View build notes <span>↗</span></button></div></article>`).join('');
